...
/**
 * @Author: skwen
 * @ClassName: AdminLoginRecordController
...
public class AdminLoginRecordController {
    ...
    public Map<String, Object> listLoginRecord(int pageNumber, int pageSize, String sortName, String sortOrder, String keyword, int state, String startTime, String endTime) throws ParseException {
        ...
        if (token != null) {
            ...
            List<AdminLoginRecord> select = adminLoginRecordService.getList(adminUser);
            PageInfo<AdminLoginRecord> pageInfo = new PageInfo<>(select);
            map.put("status", 0);
            map.put("tip", "success");
    ...
    public Map<String, Object> deleteLoginRecord(int id) {
        ...
        if (token != null) {
            ...
            adminLoginRecord.setUser_id(token.getId());
            adminLoginRecord.setId(id);
            int count = adminLoginRecordService.deleteOne(adminLoginRecord);
            if (count > 0) {
                map.put("status", 0);
                map.put("tip", "success");
            } else {
    ...
    @LoginToken
    @RequestMapping("batchDeleteLoginRecord")
    public Map<String, Object> batchDeleteLoginRecord(int []  ids) {
        Map<String, Object> map = new HashMap<>();
        Token token = new Token();
        token = tokenService.parseToken(request.getHeader("Authorization"));
        if (token != null) {
            AdminLoginRecord adminLoginRecord = new AdminLoginRecord();
            adminLoginRecord.setUser_id(token.getId());
            adminLoginRecord.setIds(ids);
            int count = adminLoginRecordService.deleteMore(adminLoginRecord);
            if (count > 0) {
                map.put("status", 0);
                map.put("tip", "success");
                map.put("count",count);
