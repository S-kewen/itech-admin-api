...
public class YuntechFlowController {
    ...
    public Map<String, Object> listLoginRecord(int pageNumber, int pageSize, String sortName, String sortOrder, String keyword, int type, String startTime, String endTime) throws ParseException {
        ...
        if (token != null) {
            ...
            if (type == 0) {
                select = yuntechFlowService.getList(adminUser);
            } else {
                select = yuntechFlowService.getListByDay(adminUser);
            }
    ...
    @LoginToken
    @RequestMapping("batchDeleteYuntechFlow")
    public Map<String, Object> batchDeleteYuntechFlow(int []  ids,boolean deleted) {
        Map<String, Object> map = new HashMap<>();
        Token token = new Token();
        token = tokenService.parseToken(request.getHeader("Authorization"));
        if (token != null) {
            YuntechFlow yuntechFlow = new YuntechFlow();
            yuntechFlow.setIds(ids);
            yuntechFlow.setDeleted(deleted);
            int count = yuntechFlowService.deleteMore(yuntechFlow);
            if (count > 0) {
                map.put("status", 0);
                map.put("tip", "success");
                map.put("count",count);
    ...
    public Map<String, Object> deleteYuntechFlow(int id,boolean deleted) {
        Map<String, Object> map = new HashMap<>();
        Token token = new Token();
        token = tokenService.parseToken(request.getHeader("Authorization"));
        if (token != null) {
